...
| Tool | What it does |
|---|---|
| `get_skill` | Lumae's guides, one at a time: `demo-direction` (what to film and what to cut), `recording` (the capture loop, pacing, the traps) and `editing` (the project model and the pass after a take). Their names and one-line summaries are in the tool's own description, so choosing one costs no call. |
